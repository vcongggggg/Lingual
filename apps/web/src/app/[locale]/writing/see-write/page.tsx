...
import SeeWriteExercise from '@/components/writing/SeeWriteExercise';
...
export default function SeeWritePage() {
  ...
  const selectedPrompt =
    MASTER_WRITING_PROMPTS.find((p) => p.id === promptId && p.mode === 'see-write') ||
    MASTER_WRITING_PROMPTS.find((p) => p.mode === 'see-write') ||
  ...
  return (
    <main className="min-h-screen pb-20 pt-6 px-4 sm:px-6 max-w-7xl mx-auto space-y-6 pointer-events-auto">
      ...
      <SeeWriteExercise prompt={selectedPrompt} locale={locale} />
    </main>
  );
}
