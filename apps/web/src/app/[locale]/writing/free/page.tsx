...
import FreeWritingExercise from '@/components/writing/FreeWritingExercise';
...
export default function FreeWritingPage() {
  ...
  const selectedPrompt = MASTER_WRITING_PROMPTS.find(
    (p) => p.id === promptId && p.mode === 'free'
  );
  ...
  return (
    <main className="min-h-screen pb-20 pt-6 px-4 sm:px-6 max-w-7xl mx-auto space-y-6 pointer-events-auto">
      ...
      <FreeWritingExercise prompt={selectedPrompt} locale={locale} />
    </main>
  );
}
