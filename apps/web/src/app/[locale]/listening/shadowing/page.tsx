...
import { ShadowingResult } from '@linguaflow/domain';
...
export default function ShadowingPage() {
  ...
  const exercises = SAMPLE_LISTENING_EXERCISES.filter((ex) => ex.modes.includes('shadowing'));
  ...
  const handleComplete = (res: ShadowingResult) => {
    ...
    <main className="min-h-screen pb-20 pt-6 px-4 sm:px-6 max-w-4xl mx-auto space-y-6 pointer-events-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="inline-flex items-center gap-1.5 text-xs font-bold text-amber-400">
          ...
          <span>Nhại giọng (Shadowing)</span>
        </div>
      </div>
